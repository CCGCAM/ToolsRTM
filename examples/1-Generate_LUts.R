...
#inputsSCOPE = read.table('examples/LUTs/inputs_SCOPE.csv', sep=',', header = T)
#inputsPRO = read.table('examples/LUTs/inputs_PROSAIL.csv', sep=',', header = T)
#inputsINF = read.table('examples/LUTs/inputs_INFORM.csv', sep=',', header = T)
#save(inputsINF, file='data/InputsLUT_INFORM.rda')

inputs = ToolsRTM::inputsSCOPE
data.ICOS = ToolsRTM::data.ICOS

LUt_time<-getLUT_time(inputs = inputs, dataICOS = data.ICOS,timeStart = '2018-01-01', 
                      timeEnd = '2018-12-31',freq.hour = 'diurnal',
                      SetFixedSeed = T,nLUT = 10)
head(LUt_time)
