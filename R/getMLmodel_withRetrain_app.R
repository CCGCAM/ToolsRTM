...
#' @param n.layers  number of layer By default is 3
...
#'
getMLmodel.withRetrain_app <-function(dataset=NULL, depVar='Cab',model='CNN',optimizer='adam',
         n.times=NULL,
  ...
  progress_callback <- callback_lambda(on_epoch_end = function(epoch, logs) {
    # Update progress based on epoch
    if (!is.null(session)) {
      incProgress(1 / n.epochs, session = session)
    }
  })

  callbacks_ = keras::callback_early_stopping(monitor = 'val_loss', mode = 'min', patience = 5, restore_best_weights = TRUE)
  # Combine the callbacks into a single list
  callbacks_list <- list(progress_callback)
  ...
  if (exists('optimizer') == FALSE) {
    ...
    opt.retrain =  keras::optimizer_adam(learning_rate = 0.00001,beta_1 = 0.9, beta_2 = 0.999)
  } else {
    opt = optimizer
  }
  ...
  for (i.times in c(1:N.times)){
    ...
    if (model == 'Hidden-layers') {
      ...
      if (depVar.trans == FALSE) {
        ...
        df.val<-getPredicts(model=model.dML, type.model='Hidden-layers',
        ...
        df.val <- df.val[, colSums(is.na(df.val)) != nrow(df.val)]
        colnames(df.val) <- c(depVar,paste(depVar,'.predicted',sep=''))
        df.val[,1] <-getReverse.trans(preProc=scaler.depVar,data=as.matrix(df.val[,depVar]))
      ...
      dim(data.to.retrain)
      split.retrain<-getSplitData_noMessages(data=data.to.retrain, depVar=depVar,inputs=inputs_[-1],
      ...
      if (depVar.trans == FALSE) {
        ...
        df.val.retrain<-getPredicts(model=model.dML, type.model='Hidden-layers',
                                              data=split.data[['Xval']], data.trans=data.trans,
                                              data.Y=split.data[['Yval']],
                                              depVar=depVar)
        ...
        df.val.retrain <- df.val.retrain[, colSums(is.na(df.val.retrain)) != nrow(df.val.retrain)]
        colnames(df.val.retrain) <- c(depVar,paste(depVar,'.predicted',sep=''))
        df.val.retrain[,1] <-getReverse.trans(preProc=scaler.depVar,data=as.matrix(df.val.retrain[,depVar]))
      ...
      if (depVar.trans == FALSE) {
        ...
        df.val<-getPredicts(model=model.dML, type.model='CNN',
                                      data=split.data[['Xval']], data.trans=data.trans,
                                      data.Y=split.data[['Yval']],
                                      depVar=depVar)
        df.val<- df.val[, colSums(is.na(df.val)) != nrow(df.val)]
      ...
      # fit the configuration for the model
      history.model.dML<- model.dML %>% fit(data.Xtrain.reshape, split.retrain[['Ytrain']],
                                            epochs = floor(n.epochs/2), batch_size = floor(batch.size/2), verbose=1,shuffle=F,
                                            #callbacks =callbacks_,
      ...
      if (depVar.trans == FALSE) {
        ...
        df.val.retrain<-getPredicts(model=model.dML, type.model='CNN',
                                              data=split.data[['Xval']], data.trans=data.trans,
                                              data.Y=split.data[['Yval']],
                                              depVar=depVar)
        df.val.retrain<- df.val.retrain[, colSums(is.na(df.val.retrain)) != nrow(df.val.retrain)]
    ...
    rmse.se<-round(RMSE(df.val.retrain[,depVar],df.val.retrain[,y.predicted]),2)
    ...
    scatter.model <-  ggplot(df.val.retrain, aes_string(x=depVar, y=y.predicted)) +
      geom_point(size = 1, alpha = 0.5) + xlim(0,max_) + ylim(0,max_)+
  ...
  if (depVar.trans == FALSE) {
    model.outputs<- list('model' = models.keras,
