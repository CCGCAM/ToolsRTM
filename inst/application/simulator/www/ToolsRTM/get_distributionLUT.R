   ...
   for (i in 1:length(minval)){
      ...
      if(TypeDistrib[[trait]] == 'Uniform') {
        ...
        inputLUT[[trait]] <- stats::runif(nSamples,min = minval[1,trait],max=maxval[1,trait])
         ...
         inputLUT[[trait]] <- correlatedValue(x=inputLUT[['Cab']]/4, r=.8)
         }
      ...
      inputLUT[[trait]] <- gauss_byMin_Max(n=nSamples, m=Mean_gauss[1,trait], s=std_gauss[1,trait], lwr=minval[1,trait], upr=maxval[1,trait], nnorm=n_casesNorm)
      if (names(inputLUT)[i] == 'Car' & DepCab == T){
         set.seed(value_seed)
         inputLUT[[trait]] <- correlatedValue(x=inputLUT[['Cab']]/4, r=.8)
      }
