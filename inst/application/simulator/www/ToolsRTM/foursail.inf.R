...
#' \code{foursail.inf} The foursail (or 4SAIL) radiative transfer model is  a RT model uses for simulating bidirectional
...
#'
foursail.inf<- function(inputLUT,rsoil=r_understorey, rleaf=r_leaf,tleaf=t_leaf,short.waves=T){
  ...
  if (lai<0){
    ...
    NonConScatt <- NonConservativeScattering(m,lai,att,sigb,ks,ko,sf,sb,vf,vb,tss,too)
  ...
  if (short.waves == T){
    r_BRF<-Compute_BRF(rdot=rdot,rsot=rsot,tts=tts,data.light=dataSpec_PDB,short.waves = T)
  } else {
    r_BRF<-Compute_BRF(rdot=rdot,rsot=rsot,tts=tts,data.light=dataSpec_PDB)
  }
