...
#' sim <-getFluspect.Cx(inputsLeaf = LUT, inputsOptipar =optipar,version='Cx')
#'
...
getFluspect.Cx<-function(inputsLeaf,inputsOptipar, version = 'Cx' )  {
  ...
  if (missing(inputsOptipar)) {
    nr = optipar[['nr']]; Kdm = optipar[['Kdm']];Kab = optipar[['Kab']];

    Kca = optipar[['Kca']]; Kw = optipar[['Kw']]; Ks = optipar[['Ks']];
    Kant = optipar[['Kant']];
    Kcbc = optipar[['Kcbc']]; Kp = optipar[['Kp']];
    phiI = optipar[['phiI']]; phiII = optipar[['phiII']];
  ...
  if (Cx == -999) {
    ...
    Kca <- (1 - Cx) * optipar[['KcaV']] + Cx * optipar[['KcaZ']]
  }
