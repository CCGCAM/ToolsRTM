  ...
  if ( is.null(model) | model == 'PROSAIL'){
    ...
    ### simulations
      sims<-foreach(i=1:nLUT) %dopar% {
        data.foursail_pro<-foursail(inputLUT=LUT[i,],rsoil=rsoil,LeafModel = 'PROSPECT-PRO')
        rdot<-data.foursail_pro[[1]]
        rsot<-data.foursail_pro[[2]]
        rfl.prosail<-Compute_BRF(rdot=rdot,rsot=rsot,tts=LUT[i,'tts'],data.light=dataSpec_PDB)
    ...
    sim.rfl<-list()
      ### simulations
      sims<-foreach(i=1:nLUT) %dopar% {
        rfl.prospect<-prospect_PRO(N=LUT[i,'N'],Cab=LUT[i,'Cab'],Car=LUT[i,'Car'],
    ...
    if (is.null(method) | method == 'ggplot' | method != 'classical' ){
      ...
      ### simulations
        sims<-foreach(i=1:nLUT) %dopar% {
          rfl.inform<-inform(inputLUT = LUT[i,],rsoil=rsoil,LeafModel = 'PROSPECT-PRO')
