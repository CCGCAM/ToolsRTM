...
getLUT<-function(inputs=NULL, nLUT=100, setseed = 123){
  ...
  if (length(missing_columns) > 1) {
    ...
    message('Check default tables in inputs; where inputs are: inputsFlUSPECT; inputsSCOPE;, inputsSPART; inputsINFORM; inputsLiberty; inputsPROSAIL; inputsRTMs ')
  ...
  for (i in  c(1:length(inputs_names))) {
    ...
    if (table.sb[,'use.default'] == 1){
      ...
      if (table.sb[,'Distribution'] == 'Uniform'){
        ...
        n_casesNorm= 3 * nLUT
        var.list[[trait]] <-gauss_byMin_Max(n=nLUT, m=as.numeric(table.sb[,'Mean_D']),
  ...
  var.list[[trait_dep]] <- correlatedValue(x=var.list[['Cab']]/4, r=.8)
