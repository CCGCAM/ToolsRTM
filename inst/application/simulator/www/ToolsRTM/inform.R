...
#r_understorey <- msail_background(inputLUT=inputLUT,rsoil=refl_soil,LeafModel= LeafModel, typeLAI = 'understorey')
if (LeafModel == 'Fluspect-B' | LeafModel == 'Fluspect-B-Cx' ) {


  r_understorey <- foursail.inform(inputLUT=inputLUT,rsoil=refl_soil, typeLAI = 'understorey')
...
if (LeafModel == 'PROSPECT-PRO') {
  ...
  LRT <- getFluspect.B(inputsLeaf= inputLUT, inputsOptipar =optipar,version='D')
  ...
  LRT <- getFluspect.Cx(inputsLeaf= inputLUT, inputsOptipar =optipar,version='Cx')
  message('SAIL with fluspect model is processing')
  r_leaf <- LRT[[2]] #rho Reflectance
  t_leaf <- LRT[[3]] #tau Transmittance
...
if (LeafModel == 'Fluspect-B' | LeafModel == 'Fluspect-B-Cx' ) {
  ...
  r_sail_inf <- foursail.inf(inputLUT=inputLUT,rsoil=r_understorey,rleaf=r_leaf,tleaf=t_leaf)
}
...
t_s <- foursail_t_s(inputLUT=inputLUT,rsoil=r_understorey,rleaf=r_leaf,tleaf=t_leaf)


# Crown transmittance in observation direction (t_o)
t_o <- foursail_t_o(inputLUT=inputLUT,rsoil=r_understorey,rleaf=r_leaf,tleaf=t_leaf)
