...
getMLmodel<-function(dataset=NULL, depVar='Cab',model='CNN',optimizer='adam',
  ...
  split.data<-getSplitData(data=dataset[,inputs_], depVar=depVar,inputs=inputs_[-1],
  ...
  if (exists('model.dML')){
    ...
    if (depVar.trans == FALSE) {
      df.plot<-getPredicts(model=model.dML, type.model=model,
      ...
      df.plot <- df.plot[, colSums(is.na(df.plot)) != nrow(df.plot)]
      colnames(df.plot) <- c(depVar,paste(depVar,'.predicted',sep=''))
      df.plot[,1] <-getReverse.trans(preProc=scaler.depVar,data=as.matrix(df.plot[,depVar]))
    ...
    rmse.stats<-round(RMSE(df.plot[,depVar],df.plot[,depVar.predicted]),2)
