...
rmse.train<-round(RMSE(pred.train,data.train[,input]),2)
mae.train<-round(MAE(pred.train,data.train[,input]),2)
#### Skill scores for testing data
r2.test<-round(cor(pred.test,data.test[,input],use='pairwise.complete.obs')^2,2)
rmse.test<-round(RMSE(pred.test,data.test[,input]),2)
mae.test<-round(MAE(pred.test,data.test[,input]),2)
...
if (is.null(Field.data)) {
  ...
  rmse.obs<-round(RMSE(predict.obs,Field.data[,paste(input,acron,sep='')]),2)
  mae.obs<-round(MAE(predict.obs,Field.data[,paste(input,acron,sep='')]),2)
