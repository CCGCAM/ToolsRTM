...
#' @param canopy.model Selection of canopy model options available are 'fourSAIL', 'fourSAIL' and 'INFORM'. By default fourSAIL model will be used.
...
get.SCOPE<-function(LUT, n.LUT=1,options.SCOPE,
                    optipar,path.out,
  ...
  #data.opts =  read.table('www/SCOPE/setoptions.csv',header=T,sep=',')
  ...
  for (sim.i in  c(1:nSamples)){
    ...
    #progress <- (sim.i / nSamples) * 100

    setTxtProgressBar(progress_bar, sim.i)
    #setTxtProgressBar(progress_bar, sim.i, label = paste0(progress, "% (", i, "/", nSamples, ")"))

    #print(paste("Simulation", sim.i, "/", dim(LUT)[1]))

    # Get main inputs
    data.leafbio<-getinputLUT(inputLUT=LUT[sim.i,], dataset='leafbio')
    data.canopy<-getinputLUT(inputLUT=LUT[sim.i,], dataset='canopy')
    data.meteo<-getinputLUT(inputLUT=LUT[sim.i,], dataset='meteo')

    data.soil<-getinputLUT(inputLUT=LUT[sim.i,], dataset='soil',
                                     calc.heat = calc.heat,
                                     calc.rss_rbs =  calc.rss_rbs)

    data.timeseries<-getinputLUT(inputLUT=LUT[sim.i,], dataset='timeseries') ## this is xyt in SCOPE
    data.angles<-getinputLUT(inputLUT=LUT[sim.i,], dataset='angles')
    data.mly<-getinputLUT(inputLUT=LUT[sim.i,], dataset='mly')
    ...
    data.timeseries<-getinputLUT(inputLUT=LUT[sim.i,], dataset='timeseries')
