               ...
               sidebarLayout(
                 ...
                 sidebarPanel(
                   ...
                   conditionalPanel(
                     ...
                     sliderInput("Cab_spart", "Chlorophyll content (μg cm-2)", min = 0.01, max = 100, value = 50, step = 0.1),
                     sliderInput("Car_spart", "Carotenoid content (μg cm-2)", min = 0, max = 10, value = 2.5, step = 0.1),
                     sliderInput("Anth_spart", "Anthocyanin content (μg cm-2)", min = 0, max = 7, value = 2, step = 0.1),
                     ...
                     sliderInput("EWT_spart", "Water content (g cm-2)", min = 0.001, max = 0.05, value = 0.01, step = 0.005),
                     sliderInput("Prot_spart", "Proteins (g cm-2)", min = 0.0001, max = 0.03, value = 0.012, step = 0.005),
                     sliderInput("CBC_spart", "Carbon-based Constutient (g cm-2)", min = 0, max = 0.03, value = 0.010, step = 0.005)
                   ...
                   conditionalPanel(
                     ...
                     sliderInput("LAI_spart", "LAI (m2/m2)", min = 0, max = 8, value = 4, step = 0.1),
                     sliderInput("LIDFa_spart", "LIDFa", min = 0.10, max = 90, value = 0.5, step = 0.1),
                     ...
                     sliderInput("psi_spart", "psi (deg)", min = 0, max = 180, value = 0, step = 0.5)
                   ...
                   tabsetPanel(
                     ...
                     tabPanel("Reflectance",
                            ...
                            # materialSwitch(inputId = "checkbox_scope", label = "Accumulate reflectance spectra", status = "danger"),
                              ...
                              br(),
                              br(),
                              verbatimTextOutput("lut_spart_output_leaf"),
                              strong("Plant traits and geometric values used for the selected canopy model:"),
                              br(),
                              br(),
                              verbatimTextOutput("lut_spart_output_canopy"),
                     ...
                     tabPanel("About SPART", "",
                              wellPanel( style = "background: white",
                                         h3('ToolsRTM package'),
                                         p(HTML('<b>ToolsRTM</b> package is also designed for running the soil-plant-atmosphere radiative transfer (SPART) model (Yang et. al, 2020) for satellite measurements in the solar spectrum.')),

                                         p('The SPART model uses three computationally efficient RTMs for soil (BSM), vegetation canopies (PROSAIL) and atmosphere (SMAC), respectively. The sub-models are coupled by using the four-stream theory and the adding method.'),
                                         p('The resulting `Soil-Plant-Atmosphere Radiative Transfer model simulates directional TOA spectral observations, with all major effects included, such as sun-observer geometries and non-Lambertia reflectance of the land surface.'),
                                        ...
                                        br(),
                                         ...
                                         tags$figure(
                                           tags$img(src = "rtm_spart.png",height = "450px", width = "500px"),
                                           tags$figcaption("", style = "font-weight: bold;")
                                         ...
                                         p('Yang, P., van der Tol, C., Yin, T., & Verhoef, W. (2020). The SPART model: A soil-plant-atmosphere radiative transfer model for satellite  measurements in the solar spectrum. Remote Sensing of Environment, 247,
                                           111870.'),
...
# 5) Tab Inversion module  -----------------------------------------
                 ...
                 tabPanel(title = "Inversion Module",
                          # Add a sidebar layout
                          sidebarLayout(
                            # Add a sidebar panel
                            sidebarPanel(
                              # Add information on cranes and prompt user to use slider
                              h3("Select the inversion Method:"),
                              selectInput("models_", "Selected:",
                              ...
                              mainPanel(
                                tabsetPanel(
                                  tabPanel("Hybrid ML",
                                           h3("..."),
                                           br(),
                                           #materialSwitch(inputId = "checkbox_spectra", label = "Get plot", status = "danger"),

                                           br(),
                                        ), ## end panel
                                  tabPanel("Traditional methods",
                                           h3("---"),
                                  ...
                                  tabPanel("ML references",
                                           h3(""),
                                           tags$li(tags$span("Hybrid Methods")),
                                           tags$li(tags$span("Traditional methods")),
